...
from dataset_utils.data_aug import get_transforms, get_normalize, get_corrupt_function
...
class PairedDataset(Dataset):
    ...
    def __init__(self, files_a, files_b, transform_fn, normalize_fn, corrupt_fn=None, preload=True, preload_size=0,
                 verbose=True):
...
def load_dataset_from_config(config):
    ...
    transform_fn = get_transforms(size=config['size'], scope=config['scope'], crop=config['crop'])
