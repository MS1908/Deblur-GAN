...
class PerceptualLoss():
    ...
    def get_loss(self, fakeIm, realIm):
        ...
        f_fake = self.loss_model.forward(fakeIm)
        f_real = self.loss_model.forward(realIm)
...
def loss_factory(content_loss_type='perceptual', disc_loss_type='ragan-ls'):
    if content_loss_type == 'perceptual':
        content_loss = PerceptualLoss()
        content_loss.initialize(nn.MSELoss())
    elif content_loss_type == 'l1':
        content_loss = SimpleLoss()
        content_loss.initialize(nn.L1Loss())
    else:
        raise ValueError("Content loss [%s] not recognized." % content_loss_type)

    if disc_loss_type == 'gan':
        disc_loss = DiscLoss()
    elif disc_loss_type == 'lsgan':
        disc_loss = DiscLossLS()
    elif disc_loss_type == 'wgan-gp':
        ...
        raise ValueError("GAN Loss [%s] not recognized." % disc_loss_type)
